...
yes | sudo wget https://releases.hashicorp.com/terraform/$TERRAFORM_VERSION/terraform_"$TERRAFORM_VERSION"_"$LINUXARQ".zip  -O /tmp/terraform.zip
yes | sudo unzip -o /tmp/terraform.zip -d /usr/local/bin/
...
export PATH="$PATH:$GO_INSTALL_PATH/go/bin:/usr/local/bin"
export GOBIN=$GOBIN
export GOPATH=$GOPATH
...
# Add gcloud command line tools to the user's PATH
echo "source /usr/share/google-cloud-sdk/completion.bash.inc" >> ~/.bashrc
echo "source /usr/share/google-cloud-sdk/path.bash.inc" >> ~/.bashrc
source ~/.bashrc
